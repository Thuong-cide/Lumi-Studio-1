...
const external = [
  ...
  "@opentelemetry/*", "@google-cloud/*", "@google/*", "firebase-admin",
...
async function buildAll() {
  ...
  // Bundle create-admin as a standalone script (TypeScript, uses same packages as the server)
  await esbuild({
    entryPoints: [path.resolve(artifactDir, "src/create-admin.ts")],
    platform: "node",
    bundle: true,
    format: "esm",
    outfile: path.resolve(distDir, "create-admin.mjs"),
    logLevel: "info",
    external,
    banner,
  });
